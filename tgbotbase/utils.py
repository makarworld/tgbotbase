...
for key, env_key in [("filename",           "LOG_FILENAME"      ),  # os.environ["LOG_FILENAME"]       = ...
                     ...
                     ("locales_startswith", "LOCALES_STARTSWITH"),  # os.environ["LOCALES_STARTSWITH"]         = ...
                     ("keyboards_path",     "KEYBOARDS_PATH"    )]: # os.environ["KEYBOARDS_PATH"]     = ...
    if ENV.get(env_key) is not None:
        utils_settings[key] = ENV[env_key] if not env_key.endswith("_BOOL") else smart_bool(ENV[env_key])
...
logger.add(utils_settings["filename"], 
           format = utils_settings["log_format"], 
...
def localizator(key: str, locale: str = 'en', **kwargs) -> str:
    return i18n.t(f"{utils_settings['locales_startswith']}.{key}", locale = locale, **kwargs)
...
def set_value_i18n(locale: str, path: str, value: str):
    data = load_yaml(f"{utils_settings['locales_folder']}/{utils_settings['locales_startswith']}.{locale}.yml")
    temp = data[locale]
    path = path.split('.')
    for key in path[:-1]:
        temp = temp[key]
    ...
    with open(f"{utils_settings['locales_folder']}/{utils_settings['locales_startswith']}.{locale}.yml", "w", encoding="utf-8") as f:
        yaml.dump(data, f)
...
def get_sender(message: Union[Message, CallbackQuery]) -> Union[EditMessageText, SendMessage]:
...
async def answer_message(text: str, 
