...
from tgbotbase.default_factories import CallbackFactory
from tgbotbase.answer import AnswerContext
from tgbotbase.keyboards import keyboard
from tgbotbase.utils import answer_message, Media, get_logger_filename, localizator, logger
...
class ThrottlingMiddleware(BaseMiddleware):
    ...
    async def check_event(self, message: Union[Message, CallbackQuery], data: dict):
        ...
        if isinstance(message, Message):
            logger.info(f"User {user} sent message: {message.text if message.text else message.caption}")
        else:
